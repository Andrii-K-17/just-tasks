  ...
  <Transition name="backdrop">
    ...
    >
      <Transition name="modal">
        <div
          class="bg-emerald-50 border border-emerald-200 rounded-2xl w-full max-w-md shadow-2xl z-70 overflow-hidden dark:bg-slate-900 dark:border-slate-800 transition-colors"
        >
          <div
          ...
          <div class="px-5 py-5 space-y-5">
            <div class="flex items-center gap-5">
              <div class="relative w-16 h-16 flex-shrink-0">
                ...
                <span
                  class="absolute inset-0 flex items-center justify-center text-sm font-semibold text-black dark:text-slate-100"
                >
              ...
              <div>
                <p class="text-gray-900 font-semibold text-2sm dark:text-slate-100">
                  ...
                  <span class="text-gray-900 font-normal text-2sm dark:text-slate-400"
                    >/ {{ taskStore.stats.total }}</span
                  >
                </p>
            ...
            <div class="grid grid-cols-3 gap-2">
              <div
                class="bg-emerald-200/30 rounded-xl p-3 text-center border border-emerald-200 dark:bg-slate-800/50 dark:border-slate-700 transition-colors"
              >
                <CheckCircleIcon
                  class="w-4 h-4 text-emerald-600 mx-auto mb-1.5 dark:text-emerald-500"
                />
                <p class="text-black font-semibold text-lg dark:text-slate-100">
                  {{ taskStore.stats.done }}
                </p>
              ...
              >
                <ExclamationTriangleIcon
                  class="w-4 h-4 text-rose-600 mx-auto mb-1.5 dark:text-rose-500"
                />
                <p class="text-black font-semibold text-lg dark:text-slate-100">
            ...
            <div class="space-y-2.5">
              <p class="text-xs text-gray-900 font-medium tracking-widest dark:text-slate-400">
                By priority
              </p>
              ...
              <div
                v-for="item in [
                  {
                    label: 'High',
                    value: taskStore.stats.byPriority.high,
                    color: 'bg-red-600 dark:bg-red-500',
                  },
                  {
                    label: 'Medium',
                    value: taskStore.stats.byPriority.medium,
                    color: 'bg-yellow-600 dark:bg-yellow-500',
                  },
                  {
                    label: 'Low',
                    value: taskStore.stats.byPriority.low,
              ...
              >
                <span class="text-xs text-gray-900 w-12 flex-shrink-0 dark:text-slate-300">{{
                  item.label
                }}</span>
                <div
                  class="flex-1 h-2 bg-green-200/90 rounded-full overflow-hidden dark:bg-slate-700"
                >
                  <div
                    :class="['h-full rounded-full transition-colors', item.color]"
                    :style="{
                      width:
                        taskStore.stats.total > 0
                ...
                <span
                  class="text-xs text-gray-900 w-4 text-right flex-shrink-0 dark:text-slate-300"
                  >{{ item.value }}</span
                >
              </div>
            </div>
